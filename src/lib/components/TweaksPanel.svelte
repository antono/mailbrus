	...
	>
		...
		<div class="twk-body">
			...
			<div class="twk-row twk-row-h">
				...
				<button type="button" class="twk-toggle" data-on={t.dark ? '1' : '0'} role="switch" aria-label="Dark mode" aria-checked={t.dark} onclick={() => set('dark', !t.dark)} data-testid="tweaks.dark-toggle"><i></i></button>
			</div>
			<div class="twk-row">
			...
			<div class="twk-row twk-row-h">
				...
				<button type="button" class="twk-toggle" data-on={t.hintBar ? '1' : '0'} role="switch" aria-label="Show key hints" aria-checked={t.hintBar} onclick={() => set('hintBar', !t.hintBar)} data-testid="tweaks.hints-toggle"><i></i></button>
			</div>
			{#if pushSupported}
				<div class="twk-sect">Notifications</div>
				<div class="twk-row twk-row-h">
					<div class="twk-lbl"><span>Enable notifications</span></div>
					<button type="button" class="twk-toggle" data-on={pushEnabled ? '1' : '0'} role="switch" aria-label="Enable notifications" aria-checked={pushEnabled} onclick={togglePush} data-testid="tweaks.notifications-toggle"><i></i></button>
				</div>
			{/if}
		</div>
	</div>
