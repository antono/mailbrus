...
Provides a `NotmuchIndex` capability in `mailbrus-core` (gated behind a `notmuch` Cargo feature) for fast, indexed message listing by querying a notmuch database read-only, without modifying database content or triggering `notmuch new`.
