...
Provides a `SmtpSender` capability in `mailbrus-core` for sending pre-composed RFC 5322 messages via SMTP using io-smtp coroutines, supporting PLAIN authentication and STARTTLS. Credentials are accepted at the call site with no config storage.
