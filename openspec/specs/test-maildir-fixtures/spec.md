# test-maildir-fixtures

## Purpose

The pristine, committed maildir corpus and its typed manifest used by the E2E suite. Defines the corpus's read-only nature, its account/folder structure, and the message-state, attachment, mailing-list, and signature coverage it must provide, with the manifest serving as the source of truth that stays consistent with the on-disk corpus.
